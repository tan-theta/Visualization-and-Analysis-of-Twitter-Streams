...
//        var unit = this.svg.selectAll(".".concat(this.properties.unitPrefix).concat(uid));
//        if (!unit.empty()) {
//          var statename = this.properties.unitTitle(unit.datum());
//        console.log(statename);
//        }
        
//        console.log(selection);
        
        var current_fips = d.properties.fips;
        var current_uid = ".unit-" + current_fips;
        var current_uid_active = ".unit-" + current_fips+ ".active";
//        console.log(current_uid);
        console.log((current_fips));
        
//           this.style("opacity":0.5);
//            console.log(d.properties.name);
            var currentPlace = d.properties.name;
//            console.log((this.properties.column));
            var currentDate = this.properties.column;
//            drawPie(currentPlace,currentDate);
//        var selectmap = d3.select(".units")
//        var counter = 0;
//        console.log(selectmap);
        var opaque_usa = function(d){
//            var counter = 0;
            counter++;
            var selectedMap = d3.select(current_uid);
            console.log(counter);
            if (counter %2==0){
               
                d3.select("#map").selectAll(".unit").style("opacity", "1").attr("filter", "url(#no-filter)").style("stroke-width", "0.5");
//                d3.selectAll("g.units.zoom").attr("filter", "url(#map-filter)");
                
            }
            else{
               d3.select("#map").selectAll(".unit").style("opacity", "0.4").style("stroke-width", "0.5");
                 selectedMap.style("opacity", "1")
            .style("stroke-width", "2")
            .attr("filter", "url(#map-state-filter)")
            .style("stroke", "black")
                drawPie(currentPlace,currentDate);
                drawCloud(currentPlace,currentDate);
                
                if(currentDate=="14"){
                   drawBar14(currentPlace); 
                }
                else if(currentDate=="15"){
                    drawBar15(currentPlace);
                }
                else{
                    drawBar16(currentPlace);
                }
        ...
        var opaque_spain = function(d){
//            var counter = 0;
            counter_spain++;
            var selectedMap = d3.select(current_uid);
            console.log(counter_spain);
            if (counter_spain %2==0){
               
                d3.select("#map_spain").selectAll(".unit").style("opacity", "1").attr("filter", "url(#no-filter)").style("stroke-width", "0.5");
//                d3.selectAll("g.units.zoom").attr("filter", "url(#map-filter)");
                
            }
            else{
               d3.select("#map_spain").selectAll(".unit").style("opacity", "0.4").style("stroke-width", "0.5");
                 selectedMap.style("opacity", "1")
            .style("stroke-width", "2")
            .attr("filter", "url(#map-state-filter)")
            .style("stroke", "black")
                drawPie_spain(currentPlace,currentDate);
                drawCloudd(currentPlace,currentDate);
                
                if(currentDate=="14"){
                   drawBar14_spain(currentPlace); 
                }
                else if(currentDate=="15"){
                    drawBar15_spain(currentPlace);
                }
                 else if(currentDate=="16"){
                    drawBar16_spain(currentPlace);
                }
                 else if(currentDate=="17"){
                    drawBar17_spain(currentPlace);
                }
                 else if(currentDate=="18"){
                    drawBar18_spain(currentPlace);
                }
                 else if(currentDate=="19"){
                    drawBar19_spain(currentPlace);
                }
                 else if(currentDate=="20"){
                    drawBar20_spain(currentPlace);
                }
                 else if(currentDate=="21"){
                    drawBar21_spain(currentPlace);
                }           
                else if(currentDate=="22"){
                    drawBar22_spain(currentPlace);
                }
                ;
            }
            };
     
        if (current_fips.startsWith('US')){
            opaque_usa();
        }
        else if (current_fips.startsWith('SP')){
            opaque_spain();
        }
...
//      this.svg.selectAll('g.zoom').transition().duration(750).attr('transform', "translate(".concat(x0, ", ").concat(y0, ")scale(").concat(k, ")translate(-").concat(x, ", -").concat(y, ")"));
    ...
    value: function draw(selection) {
      ...
      self.svg.append('rect').attr('class', 'background').attr('width', self.properties.width).attr('height', self.properties.height)
      ...
      var drawGeoData = function drawGeoData(geo) {
        self.geo = geo;
        self.svg.append('g').attr("filter", "url(#map-filter)").attr('class', 'units zoom').selectAll('path').data(topojson.feature(geo, geo.objects[self.properties.units]).features).enter().append('path').attr('class', function (d) {
...
function (_Geomap) {
  ...
  _createClass(Choropleth, [{
    ...
    value: function drawLegend() {
      ...
      legend.append('rect').attr('class', 'legend-bg').attr('width', wBox).attr('height', hBox); // Draw a rectangle around the color scale to add a border.
      ...
      if (self.properties.domain) {
        ...
        maxDisplay = self.properties.domain[0];
      ...
      sg.selectAll('text').data(colors).enter().append('text').text(function (d, i) {
        // The last element in the colors list corresponds to the lower threshold.
          ...
          var arr = [0,1000,5000,10000];
          var count = c;
          c--;
          if (arr.includes(self.properties.domain[count])){
              return (self.properties.domain[count]);
          }
      ...
      sg.append('text').text(function () {
        return (self.properties.domain[20])
